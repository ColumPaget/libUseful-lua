...
MODS=stream.so terminal.so filesys.so process.so net.so entropy.so syslog.so hash.so sys.so time.so strutil.so dataparser.so oauth.so rawdata.so securemem.so xml.so libuseful_errors.so units_of_measure.so smtp.so
...
smtp.so: smtp_wrap.o
	$(CC) $(FLAGS) -shared -o smtp.so smtp_wrap.o $(LIBS)
	
smtp_wrap.o: smtp_wrap.c
	$(CC) $(FLAGS) -c smtp_wrap.c -o  smtp_wrap.o

smtp_wrap.c: smtp.i
	swig $(DEFS) -lua -o smtp_wrap.c smtp.i
